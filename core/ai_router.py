"""
core/ai_router.py — provider-agnostic AI layer (additive, no rewrites).

Existing main.py keeps using google-genai directly. New code can route
through ModelRouter: primary -> fallback -> graceful error, with
timeouts/retries. API keys are never logged.

Env (optionally overridden by config/api_keys.json gemini key):
  AI_PROVIDER, AI_BASE_URL, AI_API_KEY, AI_MODEL,
  AI_FALLBACK_MODEL, AI_VISION_MODEL, AI_TIMEOUT_S, AI_MAX_RETRIES
"""
...
def _redact(s: str) -> str:
    # Never expose keys in logs/errors: show length only.
    if not s:
        return "<empty>"
...
class ProviderConfig:
    ...
    def from_env(cls) -> "ProviderConfig":
        ...
        if not cfg.api_key:
            # Fall back to the existing local config file (internal read,
            # never logged). Keeps current installs working with no .env.
            try:
...
class ContextManager:
    """Bounded sliding-window context (chars, not tokens — no tokenizer dep)."""
...
class ModelRouter:
    """Primary -> fallback -> graceful error. No network here — callers
    inject `call_fn(model, prompt, timeout)` so this stays testable and
    provider-agnostic (OpenAI-compatible or Gemini)."""
